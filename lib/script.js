...
module.exports = class CovScript {
  ...
  _recordFunction (name, startLine, startCol, endLine, endCol, count) {
    const func = new CovFunction(
      name,
      startLine,
      startCol,
      endLine,
  ...
  merge (script) {
    this.lines.forEach((line, i) => {
      this.lines[i].count += script.lines[i].count
    })
    script.functions.forEach((mfunc) => {
      const func = this.functionsMap[mfunc.name]
      if (func) {
        func.count += mfunc.count
      } else {
        this.functionsMap[mfunc.name] = mfunc
        this.functions.push(mfunc)
      }
    })
  }
}
