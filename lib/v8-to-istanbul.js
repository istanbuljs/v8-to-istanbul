...
module.exports = class V8ToIstanbul {
      ...
      block.ranges.forEach((range, i) => {
        ...
        lines.forEach(line => {
          ...
          if(this.sourceMap && path) {
            const sourceLineStart = this.sourceMap.allGeneratedPositionsFor({ source: path, line: line.line, column: 0 });
            if(!sourceLineStart.length && line.startCol !== line.endCol) {
              line.count = 0;
              return;
            }
          }
  ...
  _maybeRemapStartColEndCol (range) {
    ...
    if (this.sourceMap) {
      ...
      path = source;
      let matchingSource = this.covSources.find(covSource => covSource.path === path);
