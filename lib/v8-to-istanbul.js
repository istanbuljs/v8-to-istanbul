...
module.exports = class V8ToIstanbul {
  ...
  _maybeRemapStartColEndCol (range, isEmptyCoverage) {
    ...
    if (this.sourceMap) {
      ...
      // The returned source already handles the sourceRoot
      ...
      const matchingSource = this.covSources.find(covSource => {
        const covSourcePath = this.sourceMap.sourceRoot ? resolve(this.sourceMap.sourceRoot, covSource.path) : covSource.path
        return covSourcePath === source
      })
